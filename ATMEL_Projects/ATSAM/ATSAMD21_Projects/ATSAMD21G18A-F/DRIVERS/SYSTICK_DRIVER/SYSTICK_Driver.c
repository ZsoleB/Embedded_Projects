...
void SYSTICK_Driver_Init()
{
	...
	SYSTEM_SYSTICK_DRIVER_REG->SYSTEM_SYSTICK_CSR |= (0x01 << 0x02);

	/*Init the Systick current value register*/
	SYSTEM_SYSTICK_DRIVER_REG->SYSTEM_SYSTICK_CVR = 0x00;

	/*Set the load register with the configured value*/
	SYSTEM_SYSTICK_DRIVER_REG->SYSTEM_SYSTICK_RVR = ((SYSTICK_DRIVER_LOAD & 0xFFFFFFUL) - 1);
...
void SYSTICK_Driver_Start()
{
	...
	SYSTEM_SYSTICK_DRIVER_REG->SYSTEM_SYSTICK_CSR|=0x01;
}
...
uint32 SYSTICK_Driver_GetClockValue()
{
	...
	Systick_Cnt_value = SYSTEM_SYSTICK_DRIVER_REG->SYSTEM_SYSTICK_CVR;
...
{
	uint8 result = 0x00;
	result = (SYSTEM_SYSTICK_DRIVER_REG->SYSTEM_SYSTICK_CSR & (0x01<<0x10))?0x01:0x00;
	return result;
}
