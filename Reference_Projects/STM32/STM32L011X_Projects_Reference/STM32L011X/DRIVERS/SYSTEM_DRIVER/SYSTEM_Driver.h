...
of an exception handler it returns to Thread mode(normal mode, the other option is Handler mode
which is entered in case of an exception/interrupt. These can differ, if configured on their
access level to certain registers(privileged or unprivileged mode).Read the respective programming
manual for more info) and immediately enters sleep mode. Use this mechanism in applications that
